...
class TestIsMediaPlaying:
    """_is_media_playing() wraps nowplaying-cli."""
...
class TestPauseMedia:
    ...
    @patch("heyvox.audio.media._is_media_playing", return_value=None)
    def test_uses_media_key_when_no_session(self, mock_state, mock_key):
        result = media.pause_media()
        assert result is True
        mock_key.assert_called_once()
        assert os.path.exists(media._PAUSE_FLAG)
        assert open(media._PAUSE_FLAG).read() == "key"

    @patch("heyvox.audio.media._is_media_playing", return_value=False)
    def test_noop_when_already_paused_by_user(self, mock_state):
        result = media.pause_media()
    ...
    @patch("heyvox.audio.media._is_media_playing", return_value=True)
    def test_uses_mediaremote_when_playing(self, mock_state, mock_mr, mock_key):
        mr_lib = MagicMock()
    ...
    @patch("heyvox.audio.media._is_media_playing", return_value=True)
    def test_falls_back_to_media_key_when_mr_unavailable(self, mock_state, mock_mr, mock_key):
        result = media.pause_media()
        assert result is True
        mock_key.assert_called_once()
...
class TestResumeMedia:
    ...
    def test_resumes_via_media_key(self, mock_sleep, mock_key):
        with open(media._PAUSE_FLAG, "w") as f:
            f.write("key")
    ...
    @patch("heyvox.audio.media._get_mr")
    @patch("heyvox.audio.media.time.sleep")
    def test_resumes_via_mediaremote(self, mock_sleep, mock_mr):
        mr_lib = MagicMock()
        mr_lib.MRMediaRemoteSendCommand.return_value = True
        mock_mr.return_value = mr_lib
        with open(media._PAUSE_FLAG, "w") as f:
...
class TestFlagLifecycle:
    ...
    @patch("heyvox.audio.media._send_media_key", return_value=True)
    @patch("heyvox.audio.media._is_media_playing", return_value=None)
    def test_pause_creates_flag(self, mock_state, mock_key):
        assert not os.path.exists(media._PAUSE_FLAG)
        media.pause_media()
        assert os.path.exists(media._PAUSE_FLAG)
    ...
    @patch("heyvox.audio.media._send_media_key", return_value=True)
    @patch("heyvox.audio.media.time.sleep")
    def test_resume_removes_flag(self, mock_sleep, mock_key):
        with open(media._PAUSE_FLAG, "w") as f:
            f.write("key")
        media.resume_media()
        assert not os.path.exists(media._PAUSE_FLAG)
