#!/usr/bin/env python3
"""
HeyVox Wake Word Retrain — SMOKE VARIANT
==========================================
End-to-end pipeline test with reduced steps.  Catches config / merge /
train-loop / onnx-save failures before burning a full T4 run.

Differences vs v7:
- steps: 500 (was 75000)              ~30 sec training instead of ~30 min
- augmentation_rounds: 1 (was 2)      faster feature augmentation
- output: hey_vox_smoke_complete.onnx (separate path — does NOT clobber real model)
- model_name: hey_vox_smoke           separate working dir inside /content

Everything else is identical to v7 — same merge, same config shape, same
export path.  If this finishes with a valid ONNX file, the full run is
very likely to succeed too.
    ...
    !python /content/drive/MyDrive/heyvox_training_checkpoints/retrain_heyvox_smoke.py
"""
...
# features and zero positive_test WAVs.
...
# ============================================================
# openwakeword/train.py L741-745 unconditionally reads 50 WAVs from
# positive_test/ to auto-tune total_length even when --train_model is the
# only flag. Seed 50 silent 32000-sample WAVs; total_length is pinned to
# 32000 below so the probe's computed median is a no-op.
...
# ============================================================
print("[8/9] Starting SMOKE training (500 steps, ~30 sec on Colab GPU)...")
print("=" * 60)
print()
...
# Force 'fork' start method. Colab/Linux already defaults to 'fork'; this
# is belt-and-suspenders for Python 3.14+ forkserver migration or any
# future default flip. The DataLoader at train.py L864 carries a lambda
# in its worker closure that breaks under 'spawn'.
_bootstrap = (
...
model_path = None
for f in onnx_files:
    # Prefer hey_vox_smoke.onnx specifically — there may be leftover
    # hey_vox.onnx files in the output dir from prior full runs.
    if 'hey_vox_smoke' in os.path.basename(f):
        model_path = f
        break
if model_path is None:
    # Fallback: any hey_vox*.onnx (still distinguishes from openwakeword res)
    for f in onnx_files:
...
if os.path.exists(data_file) or model_size < 100_000:
    ...
    output_path = os.path.join(CHECKPOINT_DIR, 'hey_vox_smoke_complete.onnx')
...
print()
print("=" * 60)
print("SMOKE SUCCESS! Model saved to Google Drive:")
print(f"  {output_path}")
print()
print("Pipeline verified end-to-end. Safe to launch the full (75000-step) run.")
print("=" * 60)
