#!/usr/bin/env python3
"""
HeyVox Wake Word Retrain Script v8
====================================
    # Cell 1:
...
- v8: add a SECOND negative class `speech_hard_negative` — windowed LibriSpeech
      connected speech (tools/featurise_speech_negatives.py -> speech_negatives.tar.gz).
      The 5.4h gate eval (tools/fp_rate_eval.py) showed the deployed model fires
      at score ~1.0 on ordinary speech with no shared "hey vox" homophone: an
      over-broad positive boundary from too few natural-speech negatives. Kept as
      its own class (separate batch weight) so generic speech doesn't dilute the
      user's specific runtime TN/FP in personal_hard_negative. If the speech
      tarball is absent, v8 degrades gracefully to exactly the v7 behaviour.
"""
...
# v8: windowed connected-speech hard-negatives (LibriSpeech), produced locally
# by tools/featurise_speech_negatives.py and uploaded to Drive. Ingested as its
# OWN negative class so its per-batch weight is tuned independently of the
# user's ambient TN/FP. If the tarball is absent, v8 trains exactly like v7.
SPEECH_FEATURES_TAR = os.path.join(CHECKPOINT_DIR, 'speech_negatives.tar.gz')
SPEECH_HN_PATH = os.path.join(MODEL_SUBDIR, 'speech_hard_negative_features.npy')
SPEECH_HN_BATCH_N = 256  # per-batch samples from the speech-negative class
...
# ============================================================
# STEP 3.6: Merge speech hard-negatives (v8) — OPTIONAL second neg class
# ============================================================
# Windowed LibriSpeech connected speech. If the tarball isn't on Drive, v8
# falls back to v7 behaviour (speech class simply not registered in STEP 7).
print("[3.6/9] Merging speech hard-negatives (v8)...")
SPEECH_AVAILABLE = False
if os.path.exists(SPEECH_HN_PATH):
    SPEECH_AVAILABLE = True
    print("  speech_hard_negative_features.npy already present (rerun) — skipping extract.")
elif os.path.exists(SPEECH_FEATURES_TAR):
    import numpy as np
    sp_staging = os.path.join(OUTPUT_DIR, '_speech_features_staging')
    os.makedirs(sp_staging, exist_ok=True)
    with tarfile.open(SPEECH_FEATURES_TAR, 'r:gz') as tf:
        tf.extractall(sp_staging)
    sp_npy = os.path.join(sp_staging, 'speech_hard_negative.npy')
    if os.path.exists(sp_npy):
        speech_neg = np.load(sp_npy)
        if speech_neg.dtype != np.float32:
            speech_neg = speech_neg.astype(np.float32)
        np.save(SPEECH_HN_PATH, speech_neg)
        SPEECH_AVAILABLE = True
        print(f"  speech_hard_negative_features.npy written: {speech_neg.shape}")
    else:
        print(f"  WARNING: speech_hard_negative.npy missing in {SPEECH_FEATURES_TAR} — skipping.")
else:
    print(f"  No speech tarball at {SPEECH_FEATURES_TAR} — v8 runs as v7 (no speech class).")
    print("  Produce it with: python3 tools/featurise_speech_negatives.py "
          "--corpus <speech_dir> --tarball speech_negatives.tar.gz, then upload to Drive.")
...
# v8: register the speech-negative class only if its features were merged in
# STEP 3.6. Keeping it separate from personal_hard_negative gives the LibriSpeech
# connected-speech pool its own per-batch weight, directly targeting the
# over-broad positive boundary the 5.4h gate eval exposed.
if SPEECH_AVAILABLE:
    config['feature_data_files']['speech_hard_negative'] = SPEECH_HN_PATH
    config['batch_n_per_class']['speech_hard_negative'] = SPEECH_HN_BATCH_N
    print(f"  v8: speech_hard_negative class registered (batch_n={SPEECH_HN_BATCH_N}).")
else:
    print("  v8: speech_hard_negative class NOT registered (no tarball) — v7-equivalent run.")
