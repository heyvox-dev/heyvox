...
Detection & control strategy (in order):
1. nowplaying-cli + MediaRemote — for native apps (Spotify, Music, Podcasts)
2. AppleScript + Chrome JavaScript — for browser media (YouTube in Chrome)
   Requires: Chrome → View → Developer → Allow JavaScript from Apple Events
3. Fallback: Chrome tab URL detection + media key — if JS is disabled
...
# Contents: "mr" (MediaRemote), "chrome-js" (Chrome JS), "media-key" (media key toggle)
...
def pause_media() -> bool:
    ...
    Strategy:
    1. If nowplaying-cli detects active playback → use MediaRemote (precise)
    2. If media is registered but paused → don't touch (user paused it)
    3. If no native media → check browsers:
    ...
    # --- Try native media first (Spotify, Music, Podcasts) ---
    ...
    # --- No native media session: check browsers ---
