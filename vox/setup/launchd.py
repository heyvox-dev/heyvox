"""
launchd service management for vox.

Provides functions to write a launchd plist, bootstrap (start), bootout (stop),
query status, and restart the vox listener service. Designed for macOS LaunchAgents.
...
def write_plist() -> Path:
    """Generate and write the launchd plist file for the vox service.
...
def bootstrap() -> tuple[bool, str]:
    ...
    if result.returncode == 0:
        return True, f"Vox service started ({LAUNCHD_LABEL})"
...
def get_status() -> dict:
    """Query launchd for the current vox service status.

    Parses `launchctl list com.vox.listener` output.
...
def restart() -> tuple[bool, str]:
    """Stop then start the vox service.
